...
export function Colored  ({
    ...
    return (
        <TouchableOpacity onPress={onPress} disabled={isLoading ? true : disabled}>
            <Wrapper animation={animation} style={[appStyles.buttonColord, { borderRadius: sizes.buttonRadius, height: sizes.buttonHeight, backgroundColor: disabled ? colors.buttonColor + '80' : buttonColor ? buttonColor : colors.buttonColor }, buttonStyle]}>
...
export function ColoredSmall ({ text, onPress, buttonStyle, customIcon, direction, textStyle, iconName, iconType, iconSize, iconColor, iconStyle })  {
    return (
        <TouchableOpacity onPress={onPress} style={[{ borderRadius: 20, paddingHorizontal: responsiveWidth(8), paddingVertical: responsiveHeight(1), backgroundColor: colors.appColor5 }, buttonStyle]}>
            <Wrapper style={{ flexDirection: direction ? direction : 'row', alignItems: 'center' }}>
                {
                    customIcon ?
                        <Icons.Custom
